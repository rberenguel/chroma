// ColorNames.js - Curated list of interesting color names
// Subset from color-name-list for generating level names

export const COLOR_NAMES = [
  // Reds & Oranges
  { name: "100 Mph", hex: "#c93f38" },
  { name: "1989 Miami Hotline", hex: "#dd3366" },
  { name: "Ablaze", hex: "#c04641" },
  { name: "Acapulco Sun", hex: "#eb8a44" },
  { name: "Afterburner", hex: "#ff4433" },
  { name: "Agressor Orange", hex: "#ff5500" },
  { name: "Amber", hex: "#ffbf00" },
  { name: "Atomic Tangerine", hex: "#ff9966" },
  { name: "Autumn Orange", hex: "#dd5522" },
  { name: "Blazing Saddles", hex: "#dd5533" },
  { name: "Blood Orange", hex: "#fe4b03" },
  { name: "Bonfire", hex: "#ff5d00" },
  { name: "California Sunset", hex: "#ff6633" },
  { name: "Candy Apple", hex: "#ff0800" },
  { name: "Carrot Orange", hex: "#ed9121" },
  { name: "Cherry Bomb", hex: "#ff2052" },
  { name: "Chili Pepper", hex: "#c21807" },
  { name: "Cinnabar", hex: "#e34234" },
  { name: "Coral Reef", hex: "#ff7f50" },
  { name: "Crimson", hex: "#dc143c" },
  { name: "Dragon Fire", hex: "#ff4422" },

  // Yellows & Golds
  { name: "24 Karat", hex: "#dfc685" },
  { name: "Aureolin", hex: "#fdee00" },
  { name: "Banana Yellow", hex: "#ffdd00" },
  { name: "Blazing Yellow", hex: "#ffff11" },
  { name: "Butter", hex: "#ffff88" },
  { name: "Canary", hex: "#ffff99" },
  { name: "Cyber Yellow", hex: "#ffd300" },
  { name: "Dandelion", hex: "#fed85d" },
  { name: "Electric Yellow", hex: "#ffff33" },
  { name: "Gargoyle Gas", hex: "#ffdf46" },
  { name: "Gold Fusion", hex: "#ffdd00" },
  { name: "Jasmine", hex: "#f8de7e" },
  { name: "Lemon Glacier", hex: "#fdff00" },
  { name: "Maize", hex: "#fbec5d" },
  { name: "Neon Carrot", hex: "#ffa343" },
  { name: "Royal Yellow", hex: "#fada5e" },
  { name: "Sunglow", hex: "#ffcc33" },
  { name: "Unmellow Yellow", hex: "#ffff66" },

  // Greens
  { name: "Acid Green", hex: "#8ffe09" },
  { name: "Alien Armpit", hex: "#84de02" },
  { name: "Android Green", hex: "#a4c639" },
  { name: "Aquamarine", hex: "#7fffd4" },
  { name: "Atlantis", hex: "#97cd2d" },
  { name: "Chartreuse", hex: "#7fff00" },
  { name: "Ectoplasm", hex: "#8fff87" },
  { name: "Emerald", hex: "#50c878" },
  { name: "Enchanted Forest", hex: "#00563f" },
  { name: "Harlequin", hex: "#3fff00" },
  { name: "Jungle Green", hex: "#29ab87" },
  { name: "Krypton", hex: "#69ff88" },
  { name: "Lawn Green", hex: "#7cfc00" },
  { name: "Lime", hex: "#00ff00" },
  { name: "Matrix", hex: "#00ff66" },
  { name: "Mermaid", hex: "#7bfff3" },
  { name: "Mint", hex: "#3eb489" },
  { name: "Neon Green", hex: "#39ff14" },
  { name: "Radioactive", hex: "#22ff00" },
  { name: "Slime", hex: "#00ff11" },
  { name: "Spring Green", hex: "#00ff7f" },

  // Cyans & Aquas
  { name: "3AM in Shibuya", hex: "#225577" },
  { name: "Aqua", hex: "#00ffff" },
  { name: "Azure", hex: "#007fff" },
  { name: "Caribbean Current", hex: "#006d77" },
  { name: "Cyan", hex: "#00ffff" },
  { name: "Deep Aqua", hex: "#00b5ad" },
  { name: "Electric Blue", hex: "#7df9ff" },
  { name: "Fluorescent Cyan", hex: "#00ffff" },
  { name: "Frost", hex: "#00ffef" },
  { name: "Ice", hex: "#d0ffff" },
  { name: "Lagoon", hex: "#00979c" },
  { name: "Miami Teal", hex: "#00d2be" },
  { name: "Moonstone", hex: "#3aa8c1" },
  { name: "Neon Blue", hex: "#04d9ff" },
  { name: "Ocean Depth", hex: "#005a7c" },
  { name: "Paradise Island", hex: "#70ffdb" },
  { name: "Pool Water", hex: "#00ddff" },
  { name: "Robin Egg Blue", hex: "#00cccc" },
  { name: "Shocking", hex: "#00ffff" },
  { name: "Teal", hex: "#008080" },
  { name: "Turquoise", hex: "#40e0d0" },

  // Blues & Purples
  { name: "Abyssal Blue", hex: "#00035b" },
  { name: "Azure Radiance", hex: "#007fff" },
  { name: "Blurple", hex: "#5865f2" },
  { name: "Cobalt", hex: "#0047ab" },
  { name: "Deep Blue", hex: "#00008b" },
  { name: "Electric Indigo", hex: "#6f00ff" },
  { name: "Frostbite", hex: "#e936a7" },
  { name: "Galaxy", hex: "#2a2a72" },
  { name: "Heliotrope", hex: "#df73ff" },
  { name: "Indigo", hex: "#4b0082" },
  { name: "Lavender", hex: "#b57edc" },
  { name: "Midnight Blue", hex: "#191970" },
  { name: "Navy", hex: "#000080" },
  { name: "Neon Purple", hex: "#bc13fe" },
  { name: "Orchid", hex: "#da70d6" },
  { name: "Plasma", hex: "#bf00ff" },
  { name: "Purple Heart", hex: "#69359c" },
  { name: "Royal Blue", hex: "#4169e1" },
  { name: "Sapphire", hex: "#0f52ba" },
  { name: "Sunset", hex: "#fad6a5" },
  { name: "Ultramarine", hex: "#3f00ff" },
  { name: "Violet", hex: "#8f00ff" },
  { name: "Vivid Violet", hex: "#9f00ff" },
  { name: "Wisteria", hex: "#c9a0dc" },

  // Pinks & Magentas
  { name: "Bubblegum", hex: "#ffc1cc" },
  { name: "Cerise", hex: "#de3163" },
  { name: "Fuchsia", hex: "#ff00ff" },
  { name: "Hot Pink", hex: "#ff69b4" },
  { name: "Magenta", hex: "#ff00ff" },
  { name: "Neon Pink", hex: "#ff10f0" },
  { name: "Pink Flamingo", hex: "#fc74fd" },
  { name: "Psychedelic Purple", hex: "#dd00ff" },
  { name: "Rose", hex: "#ff007f" },
  { name: "Shocking Pink", hex: "#fc0fc0" },
  { name: "Synthwave", hex: "#ff1177" },
  { name: "Ultra Pink", hex: "#ff6fff" },

  // Greys & Monochrome
  { name: "Ash", hex: "#b2beb5" },
  { name: "Charcoal", hex: "#36454f" },
  { name: "Cloud", hex: "#c0c0c0" },
  { name: "Dove", hex: "#e5e4e2" },
  { name: "Graphite", hex: "#383428" },
  { name: "Iron", hex: "#606060" },
  { name: "Mercury", hex: "#e5e5e5" },
  { name: "Mist", hex: "#d5d5d5" },
  { name: "Pewter", hex: "#96a8a1" },
  { name: "Shadow", hex: "#8a795d" },
  { name: "Silver", hex: "#c0c0c0" },
  { name: "Smoke", hex: "#738276" },
  { name: "Steel", hex: "#71797e" },
  { name: "Stone", hex: "#808080" },
  { name: "Titanium", hex: "#878681" },
];
